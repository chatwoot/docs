...
<Note>
Spamming channels with repetitive messages, Hijacking threads with unrelated promotional messages isn't allowed and will be handled according to the [Code of Conduct](/contributing-guide/code-of-conduct#3-temporary-ban).
</Note>
...
Run the following command from the [Rails Console](/self-hosted/monitoring/super-admin-sidekiq).
...
Run the following script from the [Rails Console](/self-hosted/monitoring/super-admin-sidekiq).
...
<Note>
If you are still facing errors, Please check your Sidekiq worker logs or [Sidekiq UI](/self-hosted/monitoring/super-admin-sidekiq) for any errors.
</Note>
